...
3. **Asset strategy** – Include a single compressed 360 JPG (≤4K) ensured to keep overall HTML < ad platform limits. Future assets can replace the file without code changes.
4. **Testing approach** – Manual smoke tests on Android Chrome (primary) and iOS Safari (secondary). Logging hooks in controls help diagnose sensor availability quickly.
